...
namespace wavemap {
DECLARE_CONFIG_MEMBERS(
    PointcloudTopicInputConfig,
    (topic_name)(topic_type)(topic_queue_length)(measurement_integrator_names)(processing_retry_period)(max_wait_for_pose)(sensor_frame_id)(time_offset)(undistort_motion)(num_undistortion_interpolation_intervals_per_cloud)(projected_range_image_topic_name)(undistorted_pointcloud_topic_name));
...
PointcloudTopicInput::PointcloudTopicInput(
    ...
    rclcpp::Node::SharedPtr nh, rclcpp::Node::SharedPtr nh_private)
...
void PointcloudTopicInput::callback(
  ...
  if (num_points == 0) {
    RCLCPP_WARN_STREAM(
      nh_->get_logger(),"Skipping empty pointcloud with timestamp "
  ...
  if (x_field_iter == pointcloud_msg.fields.end()) {
    RCLCPP_WARN("Received pointcloud with missing field x");
    return;
  } else if ((++x_field_iter)->name != "y") {
    RCLCPP_WARN("Received pointcloud with missing or out-of-order field y");
    return;
  } else if ((++x_field_iter)->name != "z") {
    RCLCPP_WARN("Received pointcloud with missing or out-of-order field z");
    return;
  }

  // Convert to our generic stamped pointcloud format
  const uint64_t stamp_nsec = convert::rosTimeToNanoSeconds(
      pointcloud_msg.header.stamp + rclcpp::Duration(config_.time_offset));
    ...
    switch (config_.topic_type) {
      case PointcloudTopicType::kOuster:
        if (hasField(pointcloud_msg, "t")) {
          ...
          RCLCPP_WARN_STREAM(
      nh_->get_logger(),"Pointcloud topic type is set to \""
...
void PointcloudTopicInput::processQueue() {
  ...
  while (!pointcloud_queue_.empty()) {
      ...
      if (undistortion_result != PointcloudUndistorter::Result::kSuccess) {
        ...
        switch (undistortion_result) {
          ...
          case PointcloudUndistorter::Result::kIntermediateTimeNotInTfBuffer:
            ...
            break;
          default:
            RCLCPP_WARN("Unknown pointcloud undistortion error.");
        }
    ...
    RCLCPP_DEBUG_STREAM(
      nh_->get_logger(),"Inserting pointcloud with "
    ...
    RCLCPP_DEBUG_STREAM(
      nh_->get_logger(),"Integrated new pointcloud in "
