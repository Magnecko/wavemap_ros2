...
std::unique_ptr<RosInputBase> RosInputFactory::create(
  ...
  if (!input_type.isValid()) {
    RCLCPP_ERROR("Received request to create input handler with invalid type.");
    return nullptr;
  }
  ...
  switch (input_type) {
    case RosInputType::kPointcloudTopic:
      if (const auto config = PointcloudTopicInputConfig::from(params);
        ...
        RCLCPP_ERROR("Pointcloud input handler config could not be loaded.");
        return nullptr;
      }
    case RosInputType::kDepthImageTopic:
      if (const auto config = DepthImageTopicInputConfig::from(params);
        ...
        RCLCPP_ERROR("Depth image input handler config could not be loaded.");
        return nullptr;
      }
  }

  RCLCPP_ERROR_STREAM(
      nh_->get_logger(),"Factory does not support creation of input type "
                   << input_type.toStr() << ".");
  return nullptr;
}
}  // namespace wavemap
