...
DECLARE_CONFIG_MEMBERS(
    DepthImageTopicInputConfig,
    (topic_name)(topic_queue_length)(measurement_integrator_names)(processing_retry_period)(max_wait_for_pose)(sensor_frame_id)(image_transport_hints)(depth_scale_factor)(time_offset)(projected_pointcloud_topic_name));
...
void DepthImageTopicInput::processQueue() {
  ...
  while (!depth_image_queue_.empty()) {
    ...
    const rclcpp::Time stamp =
        oldest_msg.header.stamp + rclcpp::Duration(config_.time_offset);
    const auto T_W_C =
    ...
    RCLCPP_DEBUG_STREAM(
      nh_->get_logger(),"Integrated new depth image in "
