...
void RosbagProcessor::addRosbag(const std::string& rosbag_path) {
  ...
  RCLCPP_INFO_STREAM(node_->get_logger(), "Loaded rosbag " << rosbag_path);
}
...
bool RosbagProcessor::processAll() {
  ProfilerZoneScoped;
  rclcpp::Time side_tasks_last_timestamp(0);
  const rclcpp::Duration kSideTasksDt(0.01);
