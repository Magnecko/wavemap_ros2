...
class TfTransformer {
 public:
  explicit TfTransformer(FloatingPoint tf_buffer_cache_time = 10.f)
    : tf_buffer_(rclcpp::Clock::SharedPtr(new rclcpp::Clock(RCL_ROS_TIME)), tf2::Duration(static_cast<int64_t>(tf_buffer_cache_time * 1e9))),
 ...
 private:
  ...
  const rclcpp::Duration transform_lookup_retry_period_{std::chrono::duration<double>(0.02)};
  // Maximum time to wait before giving up
  const rclcpp::Duration transform_lookup_max_time_{std::chrono::duration<double>(0.25)};
