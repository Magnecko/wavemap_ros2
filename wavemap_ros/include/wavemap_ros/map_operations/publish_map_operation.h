...
class PublishMapOperation : public MapOperationBase {
 public:
  PublishMapOperation(const PublishMapOperationConfig& config,
                      ...
                      std::string world_frame, rclcpp::Node::SharedPtr nh_private);
 ...
 private:
  ...
  void publishHashedMap(const rclcpp::Time& current_time, HashedMapT* hashed_map,
                        bool republish_whole_map = false);

  rclcpp::Service republish_whole_map_srv_;
};
}  // namespace wavemap
