...
inline rclcpp::Time nanoSecondsToRosTime(uint64_t nsec) {
  ...
  return rclcpp::Time(static_cast<int32_t>(nsec / kSecToNsec),
                      static_cast<int32_t>(nsec % kSecToNsec));
}
...
inline uint64_t rosTimeToNanoSeconds(const rclcpp::Time& time) {
  constexpr uint64_t kSecToNsec = 1000000000ull;
  return static_cast<uint64_t>(time.seconds()) * kSecToNsec + time.nanoseconds();
}
}  // namespace wavemap::convert
